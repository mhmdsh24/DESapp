...
class DESApp:
    ...
    def setup_hex_entry(self, entry_widget, max_length=16):
        ...
        def show_error_message(self, entry_widget, message):
            if entry_widget == self.message_entry:
                ...
                row, col = 1, 2
                parent = self.txtinput_frame
            else:
                return

            # Check if an error label already exists
            existing_labels = [widget for widget in parent.grid_slaves(row=row, column=col) if isinstance(widget, customtkinter.CTkLabel) and widget.cget("text_color") == "red"]
            if existing_labels:
                error_label = existing_labels[0]
                error_label.configure(text=message)
            else:
                error_label = customtkinter.CTkLabel(parent, text=message, text_color="red", font=("Leelawadee UI", 12))
                error_label.grid(row=row, column=col, padx=(0, 50), pady=5, sticky='e')
    ...
    def reset(self):
        ...
        if self.details_button.cget('text')=='Hide Details':
            if hasattr(self, 'blocks_tab_view'):
                self.blocks_tab_view.grid_forget()
            elif hasattr(self, 'tab_view'):
                self.tab_view.grid_forget()    
    ...
    def process(self, operation):
        ...
        if self.input_tabview.get()=="Normal Hex":
            ...
            self.populate_sentence_rounds_tab(blocks, key, operation)
    ...
    def populate_sentence_rounds_tab(self ,blocks ,key ,operation):
        for i in range(len(blocks)):
            block_tab = self.blocks_tab_view.add(f"Block {i+1}")
            self.keymsgtabs(block_tab)
